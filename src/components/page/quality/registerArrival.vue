          ...
          <el-table
            ...
            <el-table-column label="采购单号" width="120" show-overflow-tooltip>
              <template scope="scope">
                <div>
                  <div @click="showInput(tabs.calc.list, scope.$index, 'aaEdit')">
                    <div class="ellipsis">{{ scope.row.aa }}</div>
                    <el-autocomplete
                      class="inline-input"
                      v-model="scope.row.aa"
                      :fetch-suggestions="(queryString, cb) =>querySearch(queryString, cb, tabs.calc.filter.aa, 'codeName')"
                      valueKey="codeName"
                      value="codeName"
                      placeholder="请输入内容"
                      @focus="showInput(tabs.calc.list, scope.$index, 'aaEdit')"
                      @blur="scope.row.aaEdit = false"
                      @select="(item) => scope.row.supplier = item.supplier"
            ...
            <el-table-column label="供应商名称" prop="supplier" class-name="notEdit" width="120" show-overflow-tooltip></el-table-column>
            <el-table-column label="到货日期" show-overflow-tooltip align="center" width="120">
              <template scope="scope">
                <div>
                  <div @click="showInput(tabs.calc.list, scope.$index, 'bbEdit', {})">
                    <div class="ellipsis tc">{{ scope.row.bb }}</div>
                    <el-date-picker
                      type="date"
                      size="mini"
                      placeholder="选择日期"
                      ...
                      @focus="showInput(tabs.calc.list, scope.$index, 'bbEdit', {}, false)"
                ...
                <div>
                  <div @click="showInput(tabs.calc.list, scope.$index, 'ccEdit', {})">
                    ...
                    <el-date-picker
                      ...
                      @focus="showInput(tabs.calc.list, scope.$index, 'ccEdit', {}, false)"
              ...
              <template scope="scope">
                <div>
                  <div @click="showInput(tabs.calc.list, scope.$index, 'ggEdit', {})">
                    ...
                    <el-input size="mini" v-model="scope.row.gg" @focus="showInput(tabs.calc.list, scope.$index, 'ggEdit', {})" @blur="scope.row.ggEdit = false" :style="{opacity: scope.row.ggEdit ? 1 : 0}"/>
                  </div>
                </div>
              </template>
            </el-table-column>
